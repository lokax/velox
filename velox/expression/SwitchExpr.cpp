  ...
  for (auto i = 0; i < numCases_; i++) {
    ...
    switch (booleanMix) {
      case BooleanMix::kAllTrue:
        // 所有结果都是true，计算then表达式，保存结果到result上，并且直接返回
...
bool SwitchExpr::propagatesNulls() const {
    // 感觉并不需要这么复杂，只要每个then表达式和可选的else表达式都能传播NULL值
    // 那么Switch自然就能够传播NULL值
